...
(function (root) {
        ...
        if (parseIac) {
            ...
            for (var index = 0; index < intArray.length; index++) {
                ...
                if (skip > 0) {
                    ...
                    if (nonIac > 0) {
                        Array.prototype.push.apply(sequences, regularSequence(intArray.subarray(lastIacSeq, index), options));
                    }
    ...
    function regularSequence(data, options) {
        var msg = [];
        var doBuffer = options && isString(options.buffer);
        var b = doBuffer ? options.buffer : "";
        var str = isString(data) ? b + data : b + int8a2str(data);
